function [Table,CutOffFre]=TruthTableFastConCov(InputFileName,MFCond,MFOut,NoCases,NoCC,header,HeaderOut)
...
for i=1:size(firing,2)
    Cov(i)=sum(min(firing(:,i),MFOut))/sum(MFOut);
end
Table=[Table,SS'];
Table(:,end-1)=Cov'.*SS';
